...
def visual(model, dataloader, num_classes, device='cuda'):
    all_embeddings = []
    with torch.no_grad():
        for inputs, labels, _ in tqdm(dataloader):
            inputs = inputs.to(device)
            labels = labels.to(device)
            (mu, std), features = model.S(inputs)
    ...
    for i in range(num_classes):
        ...
        plt.scatter(embeddings_2d[idxs, 0], embeddings_2d[idxs, 1], 
                    label=f'Class {i}', alpha=0.6, color=colors(i))
    plt.legend()
...
visual(model, test_loader, num_classes, device)